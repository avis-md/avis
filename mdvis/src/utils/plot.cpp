...
void plt::plot(float x, float y, float w, float h, float* dx, float* dy, uint cnt, Font* font, uint sz, Vec4 col) {
	...
	if (font) {
		...
		UI::Label(x, y + h + 2, sz, to_string_scientific(x1), font, col);
		font->Align(ALIGN_TOPRIGHT);
		UI::Label(x + w, y + h + 2, sz, to_string_scientific(x2), font, col);
		Engine::RotateUI(-90.0f, Vec2(x, y + w));
		font->Align(ALIGN_TOPLEFT);
		UI::Label(x, y + h - 4 - sz, sz, to_string_scientific(y1), font, col);
		font->Align(ALIGN_TOPRIGHT);
		UI::Label(x + w, y + h - 4 - sz, sz, to_string_scientific(y2), font, col);
