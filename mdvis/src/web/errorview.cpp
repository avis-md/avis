...
void ErrorView::Draw() {
    float y = Display::height - 18 - windowSize;
    Engine::DrawQuad(0, y, Display::width, windowSize, white(0.95f, 0.1f));
    UI::Label(5, y + 2, 12, "Compile Log", white());
    y += 20;
    for (int a = 0; a < compileMsgSz; a++) {
        auto& err = compileMsgs[a];
        if (Engine::Button(0, y + a*17, Display::width, 17, white((a == descId)? 0.1f : 0), white(0.2f), black(0.1f)) == MOUSE_RELEASE) {
            if (Input::dbclick) {
        ...
        UI::Label(Display::width * 0.1f, y + a*17, 12, "(" + std::to_string(err.linenum) + ":" + std::to_string(err.charnum) + ") " + err.msg[0], white());
    }

    if (descId > -1) {
        float y = Display::height - 18 - descSize;
        Engine::DrawQuad(0, y, Display::width, descSize, white(0.95f, 0.2f));
        int a = 0;
        ...
        UI::Label(5, Display::height - 37, 12, msg.path, white(0.7f));
    }
}