...
void AnOps::Draw() {
	...
	if (expanded) {
		...
		UI2::Toggle(expos + 2, off, w - 4, "Run On Seek", AnWeb::runOnFrame); off += 17;
		UI2::Toggle(expos + 2, off, w - 4, "High Contrast", AnWeb::highContrast);
		...
		if ((!UI::editingText && (Input::KeyUp(KEY::O))) || Engine::Button(expos - 16.f, Display::height - 34.f, 16.f, 16.f, Icons::collapse, white(0.8f), white(), white(0.5f)) == MOUSE_RELEASE)
			expanded = false;
		expandPos = std::min(expandPos + 1500 * Time::delta, w);
	}
	else {
		if ((!UI::editingText && (Input::KeyUp(KEY::O))) || Engine::Button(expos - 110.f, Display::height - 34.f, 110.f, 16.f, white(0.9f, 0.15f), white(1, 0.15f), white(1, 0.05f)) == MOUSE_RELEASE)
			expanded = true;
...
void AnOps::SendNodes(bool cp) {
	...
	DoSendNodes(AnWeb::nodesPath, "nodes/");
	message = "compiling";
...
void AnOps::SendIn() {
	...
	auto ins = IO::GetFiles(AnWeb::nodesPath + "__tmp__/in/");
	for (auto& i : ins) {
		ssh.SendFile(AnWeb::nodesPath + "__tmp__/in/" + i, path + "/ser/in/" + i);
	}
	for (auto& i : ins) {
...
void AnOps::RecvOut() {
	...
	if (!IO::HasDirectory(AnWeb::nodesPath + "__tmp__/out/")) IO::MakeDirectory(AnWeb::nodesPath + "__tmp__/out/");
	auto fls = ssh.ListFiles(path + "/ser/out/");
	for (auto& f : fls) {
		if (f[0] != '.')
			ssh.GetFile(path + "/ser/out/" + f, AnWeb::nodesPath + "__tmp__/out/" + f);
	}
}